...
*********************************************************************************
* 文件名称: lcd.c																*
* 文件简述：驱动 LCD 屏幕，支持 ILI9341(240*320)、ILI9486(320*480) 两款驱动 IC	*
* 创建日期：2024.07.07															*
* 修改日期：2024.07.21															*
* 说    明：无																	*
*********************************************************************************
...
// LCD驱动参数
uint16_t  lcd_id;			// LCD ID (支持 ILI9341 和 ILI9486)
...
{					   
   LCD_CMD = 0x29;		// 9341 与 9486 开显示命令一样
}	

// LCD 关闭显示
void LCD_DisplayOff(void)
{	
	 LCD_CMD = 0x28;	// 9341 与 9486 关显示命令一样
} 
...
void Set_Direction(uint8_t direction)
{
	...
	LCD_WriteReg(0X36, skhda|0x08);		
	LCD_Open_Window(0, 0, lcd_width-1, lcd_height-1);  // 设置完扫描方向后，开显示区域为全屏窗口
}
...
{	 
	...
	if (lcd_id == 0x9486)
	{
		LCD_CMD = setxcmd;	
		LCD_DATA = Xaddr>>8;
		LCD_DATA = 0x00FF&Xaddr;		
		LCD_DATA = Xaddr>>8;
		LCD_DATA = 0x00FF&Xaddr;
		LCD_CMD = setycmd;	
		LCD_DATA = Yaddr>>8;
		LCD_DATA = 0x00FF&Yaddr;		
		LCD_DATA = Yaddr>>8;
		LCD_DATA = 0x00FF&Yaddr;
	}
}
...
//{
// 	vuint16_t r = 0, g = 0, b = 0;
//   
//	LCD_SetCursor(x, y);	
//	
//	LCD_CMD = 0X2E;	// 9341 与 9486 读 GRAM 指令一样 
//	r = LCD_DATA; 	
//	
//	lcdm_delay(2);	  
//	b = LCD_DATA;		// 9341要读2次
...
void LCD_DrawPoint(uint16_t x,uint16_t y)
{
	...
	LCD_DATA = BRUSH_COLOR; 	// 写入颜色
}

/****************************************************************************
* 名    称: void LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
* 功    能：画线
* 入口参数：x1,y1:起点坐标
            x2,y2:终点坐标  
* 返回参数：无
...
/****************************************************************************
* 名    称: void LCD_DrawRectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
* 功    能：画矩形
* 入口参数：x1,y1:对角起点坐标
            x2,y2:对角终点坐标
* 返回参数：无
...
/****************************************************************************
* 名    称: void LCD_Color_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
* 功    能：在设置的坐标处画相应颜色（在该点写入自定义颜色）
* 入口参数：x1,y1:起点坐标
            x2,y2:终点坐标
            color 此点的颜色值
...
* 说    明：color:写入此点的颜色值   GUI调用该函数
****************************************************************************/
void LCD_Color_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
	uint16_t t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
	...
	for(t=0;t<=distance+1;t++ )//画线输出
	{  
		LCD_Color_DrawPoint(uRow,uCol,color);//颜色画点
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance)
		{
...
/****************************************************************************
* 名    称: void LCD_Color_DrawRectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
* 功    能：在设置的坐标处画相应颜色（在该点写入自定义颜色）
* 入口参数：x1,y1:对角起点坐标
            x2,y2:对角终点坐标
            color 此点的颜色值
...
* 说    明：color:写入此点的颜色值   GUI调用该函数
****************************************************************************/
void LCD_Color_DrawRectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
	LCD_Color_DrawLine(x1,y1,x2,y1,color);
	LCD_Color_DrawLine(x1,y1,x1,y2,color);
	LCD_Color_DrawLine(x1,y2,x2,y2,color);
	LCD_Color_DrawLine(x2,y1,x2,y2,color);
}
...
void LCD_Clear(uint16_t color)
{
	...
	LCD_Open_Window(0, 0, lcd_width-1, lcd_height-1);  
...
void LCD_FSMC_Config()
{
  ...
  RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC,ENABLE);//使能FSMC时钟
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;         //PC3 推挽输出,控制背光
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;     //输出模式
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //上拉
  GPIO_Init(GPIOC, &GPIO_InitStructure);            //初始化PF10 
  ...
  writeTiming.FSMC_DataSetupTime = 6;		     //数据保存时间 6ns*9个HCLK=54ns
...
{          
	...
	nlen=ex-sx+1;	
	LCD_Open_Window(sx, sy, ex - sx + 1, ey - sy + 1); 	
	LCD_WriteGRAM();     			  // 开始写入GRAM
	for(i=sy;i<=ey;i++)
	{ 
		for(j=0;j<nlen;j++)
...
{  
	...
	for (value_num = 0; value_mid > 0; value_num++)
  {
	...
	{
	 LCD_DisplayString(x,y,size,(uint8_t*)"ERROR");
	 return; //退出函数
	}
	else
	{
